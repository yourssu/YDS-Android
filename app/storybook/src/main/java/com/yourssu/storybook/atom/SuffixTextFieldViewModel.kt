...
class SuffixTextFieldViewModel(application: Application): BaseViewModel(application) {
    ...
    val suffixLabelTextString = MutableLiveData<String>("suffixLabelText")
    ...
    val onSuffixLabelTextChangeListener = object : TextField.OnTextChanged {
        override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
            s?.toString()?.let { suffixLabelTextString.value = it }
        }
    }
}